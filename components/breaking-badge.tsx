...
export function BreakingBadge() {
  return (
    <div className="bg-accent text-accent-foreground overflow-hidden" style={{ viewTransitionName: "none" }}>
      <div className="flex items-center">
        <div className="shrink-0 px-4 py-2 bg-foreground/10 font-sans text-[10px] font-semibold uppercase tracking-[1.4px] z-10 relative">
          Breaking
        </div>
          ...
          <div className="flex whitespace-nowrap animate-marquee" style={{ animationDuration: "30s" }}>
            {[...headlines, ...headlines].map((item, i) => (
              <Link
                key={i}
                href={item.href}
                className="inline-flex items-center px-6 py-2 font-sans text-[12px] font-medium hover:underline"
              >
